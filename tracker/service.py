...
class users:
    ...
    def check_start(self):
        data = cur.execute("SELECT count(id) FROM users").fetchone()
        print(data)
        if data[0] == 0:
            return True
        return False
...
class permit():
    ...
    def set_user_default_usermode(self, userid):
        try:
            upd(table='users', collumn='usermode', collumn_value='default', find_collumn='userid', find_value=userid)
            return True
    ...
    def get_user_default_usermode(self):
        data = cur.execute("SELECT userid FROM users WHERE usermode = 'default' ORDER BY id ASC LIMIT 1").fetchone()
        if not data:
            master = cur.execute("SELECT userid FROM users ORDER BY id ASC LIMIT 1").fetchone()
            self.set_user_usergroup(master[0], "admin")
            return master[0]
        return data[0]
