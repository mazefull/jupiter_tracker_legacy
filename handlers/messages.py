...
async def request_permission(message: Message, specc):
    ...
    task_id = Task().new_issue(tittle="REG_REQUEST",
                               ...
                               assign=permit().get_user_default_usermode(), project=project, task_type="SD", user=message.chat.id, deadline=0)
...
async def new_user__upd_fname(message: Message, state: FSMContext):
    ...
    if not "_" in fname:
        ...
        print('IUS ', data)
        if data['now_users']:
            permit().set_user_usergroup(userid=message.chat.id, usergroup="admin")
            if permit().set_user_default_usermode(userid=message.chat.id):
                await message.answer(f'[REG] Спасибо. Тебе выдана роль администратора\n'
                                     f'/start чтобы перейти в главное меню')
            else:
                await message.answer(f'[REG] Ошибка при назначении режима admin-default-usermode')

        else:
            print(f"TMODE: {test_mode}")
            if test_mode:
                permit().set_user_usergroup(userid=message.chat.id, usergroup="default")
                await message.answer(f'[REG][TESTMODE] Выдан доступ без запроса роли у администратора'
                                     f'/start чтобы перейти в главное меню')
            else:
